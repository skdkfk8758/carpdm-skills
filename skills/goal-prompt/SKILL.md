...
프롬프트가 거명할 사실은 전부 읽어서 확인한다 — 영향 반경에 한정, 레포 전체 아님:
...
- 소비 환경의 스킬: 자동 스킬(`tdd`·ponytail·paperthin 자동 16종)은 컨텍스트의
...
읽지 않은 파일·명령을 프롬프트에 쓰는 것은 실패다. 확인 못 한 건 갭이다. 대상 트리가
...
- **길이** — 원칙 전문을 붙여넣지 않는다. 200줄을 넘으면 Context 를 경로 포인터로.
...
보고에 싣는 것: 소비자 변형, tier/effort(런 사이징 — 잡 띄울 때 참고), 닫은 갭 수, assumption 승격 목록, 3 패밀리
