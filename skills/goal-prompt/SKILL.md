...
**"전례 0건" 을 단언하기 전에 같은 목적의 다른 메커니즘을 grep 한다.** 한 이름으로 안 나왔다고 그 능력이 없는 게 아니다 — 플랫폼은 같은 일을 하는 수단을 여럿 갖고 세대별로 이름이 다르다(실측 2026-09-03: `aws_iam_openid_connect_provider`·`oidc` 만 찾고 "IAM 자격 seam 을 새로 만들어야 한다" 고 단언했는데, 같은 파일이 이미 `eks-pod-identity-agent` 애드온을 깔고 있었다). 0건은 "그 이름이 없다" 이지 "그 능력이 없다" 가 아니다. 부재를 프롬프트에 쓸 때는 무엇을 찾아 0건이었는지 검색어를 함께 적는다.
...
- **검증 명령이 대상에 실제로 닿는지 실측한다** — 레포 문서에서 검증 명령을 복사할 때, 그 명령이 이번에 만들 파일을 훑는지 확인한 뒤 SC 에 넣는다. 두 가지가 자주 새어나간다. ① **aggregate 명령**(`kustomize build <dir>`·`tsc -p .`·전체 스위트)은 대상 목록에 등록된 것만 본다 — 신규 파일이 그 목록에 없으면 파일이 있든 없든 같은 green 이 나온다(실측 2026-09-03: `kubectl kustomize gitops/platform` 이 신규 대시보드를 안 봤다. aggregate 파일 주석은 "추가하면 여기에도 넣는다" 였는데 동종 파일 5개가 전부 빠져 있었다). ② **문자열로 내장된 설정**(ConfigMap 의 config 본문·YAML 안의 스크립트·JSON 문자열)은 스키마 검사가 그 안을 안 본다 — 그 설정을 고치는 슬라이스는 전용 문법 검사(파서·린터·괄호 균형)를 SC 에 따로 건다. 둘 중 하나라도 해당하면 그 파일을 직접 도구에 넣는 명령을 SC 에 추가한다.
