...
**사다리를 남의 설정 파일에 적용할 때의 함정** — "기본값이면 생략" 은 그 프로그램이 실제로 기본값을 채울 때만 참이다. 뺀 키가 필수인지 **소비하는 코드로 확인**한다(실측 2026-09-04: 업스트림 예시에 늘 있던 `merge_minor_cost` 를 기본값이라 보고 뺐는데, 코드가 그 키를 조건 없이 읽고 검증 단계는 기본값을 안 채웠다. 파드는 `1/1 Running` 인 채 로그 세 줄만 남기고 폴링 간격을 그대로 자서, 건강해 보이는 프로세스가 8시간 동안 메트릭 0개를 냈다). 확인이 안 되면 예시대로 두는 쪽이 사다리에 맞다 — 지운 한 줄보다 침묵 실패가 비싸다.
...
- [ ] SC 의 각 검증 명령이 **이번에 만들 파일에 실제로 닿나** — aggregate 명령의 대상 목록, 문자열 내장 설정을 안 보는 스키마 검사. 안 닿으면 BLOCKING(그 파일을 직접 넣는 명령을 추가)
- [ ] Working Method 에 BASE 신선도 확인(`git fetch` + `rev-list --count <BASE>..origin/<trunk>`)이 있나 — 없으면 BLOCKING(작성-소비 시차로 base 가 뒤처진다)
- [ ] 부재를 단언한 Context 줄에 무엇을 검색해 0건이었는지가 적혀 있나
