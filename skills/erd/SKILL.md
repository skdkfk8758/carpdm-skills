...
- **검색** — 헤더 검색창, Enter 로 매칭 테이블에 포커스 + 스크롤 이동(스키마명도 매칭).
- **스키마 구분** — 테이블에 `schema` 필드가 2종 이상이면 헤더에 스키마 필터 칩 자동
  노출(클릭 → 그 스키마만 선명, 크로스 스키마 관계선은 반투명 유지) + 카드 헤더에
  스키마 태그.
- **겹침 안전망** — 렌더 후 실측 카드 크기로 충돌을 검사해 겹친 카드를 자동으로 밀어낸다
  (수동 좌표가 카드 높이를 놓쳐도 겹침 없음). **노트(`.note`)도 고정 장애물로 포함** —
  노트와 겹친 카드는 노트 아래로 밀려나고, 노트는 z-index 로 카드 위에 그려져 주석 텍스트가
  카드 뒤로 숨지 않는다.
...
2. **적재량** — 테이블별 row count. 통계 기반 추정치가 기본(대형 테이블 count(\*) 는
   ...
   **적재량은 대상 실 DB 에서 잰 값만 싣는다** — 마이그레이션을 임시 DB 에 재적용해
   스키마를 재현한 경우(ephemeral replay), 그 DB 의 count 는 사용자의 실데이터가 아니라
   seed 뿐이다. "0건" 으로 실으면 실DB 에 데이터가 있는 사용자에게 거짓 정보가 된다 —
   이때는 전 테이블 `rows:null`(적재 정보 없음)로 두고 footer 에 사유를 명시한다.
   실 DB 접속이 실패하면 **정적 폴백 전에 사용자에게 접속 가능한 DB(dev/스테이징
   connection string)가 있는지 먼저 묻는다** — 적재량·샘플은 ERD 의 핵심 가치라
   조용히 포기하지 않는다.
...
| `{{NOTES}}` / `{{FOOTER}}` | (선택) 결정·collapse 맥락 메모 / 비고·한계·출처. **노트는 카드가 없는 빈 여백(gutter)에 배치** — 엔진이 노트를 고정 장애물로 삼아 겹친 카드를 밀지만, 카드 밀집 구역 한복판에 두면 카드가 크게 밀려 레이아웃이 흔들린다. 여백에 두면 밀림이 최소. |

**`DATA.tables` 항목** (템플릿 상단 주석에도 동일 스펙 있음):

```js
{ id:'users',            // 테이블명 그대로 — edges 가 이 id 로 참조 (오타 = 선·포커스 누락)
  schema:'platform',     // DB 스키마명. 2종 이상이면 헤더 필터 칩 + 카드 스키마 태그 자동 노출.
                         //   스키마가 하나뿐이면 생략 가능. 동명 테이블이 스키마 간 충돌하면
                         //   id 를 'schema.table' 로 유일화하고 edges 도 그 id 로 참조.
  cls:'hub',             // ''(일반) | 'hub' | 'lookup' | 'dep' — 판정 기준은 schema-discovery
  x:560, y:150,          // 카드 절대좌표 (레이아웃은 손으로 — 겹침은 렌더 후 자동 해소가 안전망)
  rows:1204,             // 적재 건수 — 대상 실 DB 측정값만. 재현(replay) DB count 금지. 모르면 null
...
3. **Artifact 게시** — Artifact 도구로 publish 한다. 게시 전 `artifact-design` 스킬
   ...
   (새 경로 = 새 URL).
### Step 4.4 — 재게시(갱신) 시나리오: 진단 라이프사이클

ERD 는 살아있는 산출물이다 — 스키마가 바뀌거나 진단이 해소되면 같은 파일 경로로
재게시해 URL 을 유지한다. 이때 두 규칙:

1. **해소된 진단은 findings 에서 제거한다 — "[해소]" 표기로 남기지 않는다.**
   진단 패널의 계약은 "현재 갭"이다(빈 배열 = 버튼 자동 숨김 = 건강한 스키마라는
   신호). 해소 항목을 접두어로 남기면 사용자는 "처리됐다더니 왜 아직 보이지?" 라고
   읽는다(실사고 피드백). 해소 *경위*는 ERD 의 일이 아니다 — spec/ADR/커밋이 그
   이력의 SSOT 이고, footer 에 한 줄 포인터("최초 진단 N건 해소 — 경위는 <경로>")만
   남긴다. 부분 해소면 남은 갭만 findings 에 유지.
2. **그리는 기준 시점을 footer 에 명시한다.** 해소 반영이 아직 실 DB 에 apply 되지
   않은 브랜치 마이그 기준이면(= 적용 후 미래 모습), "브랜치 <이름> 기준 — dev/prod
   미적용(apply 대기)" 을 헤더 부제나 footer 에 박는다. 명시 없이 그리면 ERD 가
   운영 실물로 오독된다. apply 완료 후 실 DB introspection 으로 한 번 더 갱신하는
   것이 최종 상태.

해소 반영 시 카드 쪽도 동기한다 — 컬럼 코멘트에 새 제약(CHECK 등)·재편된 값 집합을
반영하고, 발견 당시 상태를 서술하던 "⚠ ..." 류 문구는 해소 후 서술로 교체한다.
...
- **재현(replay) DB 의 count 를 적재량으로 싣기** — 마이그 재적용 임시 DB 는 스키마 소스일 뿐 데이터 소스가 아니다. seed 만 있는 DB 의 "0건" 은 실DB 사용자에게 거짓 정보(실사고 피드백). `rows:null` + footer 사유 명시가 정답.
- **다중 스키마인데 `schema` 필드 생략** — 스키마 필터 칩·카드 태그가 안 뜬다. 앱 스키마 2개 이상이면 전 테이블에 `schema` 필수.
- **자동 레이아웃(force-directed 등) 도입** — YAGNI. 위치는 손으로, 엔진은 wire·인터랙션만 자동.
- **미리 스코프 축소** — 포커스·검색이 탐색을 담당한다. 40+ 초대형만 사용자와 범위 협의.
- **노트를 카드 밀집 구역 한복판에 배치** — 겹침 해소가 노트를 장애물로 삼아 카드를 밀어내므로 여러 카드가 크게 밀려 레이아웃이 흔들린다. 노트는 빈 여백(gutter)에 둔다(실사고 피드백: 노트가 카드 뒤로 숨음 → 엔진이 노트를 고정 장애물+z-index 우선으로 처리하도록 수정).
- **근거 없는 finding 지어내기** — 진단은 실측(쿼리 결과·스키마 사실·코드 grep)이 있는 것만. 이슈 0건 = 빈 배열이 정답.
- **해소된 진단을 "[해소]" 표기로 패널에 남기기** — 패널 계약은 "현재 갭". 해소 항목은 findings 에서 제거하고 경위는 spec/ADR 포인터를 footer 에(Step 4.4 — 실사고 피드백: "처리됐다더니 왜 보이지").
- **적용 전 스키마를 기준 명시 없이 그리기** — 브랜치 마이그 기준(미적용 미래 상태)이면 그 사실을 footer 에 박는다. 없으면 운영 실물로 오독.
- **삭제 후보를 "지워도 됨" 으로 단정** — liveness 3증거 절차 없이 drop 권고 금지 (실사고 전례).
- **마이그레이션/DDL 작성** — erd 는 그리기만. 스키마 변경은 별도 턴에서.
