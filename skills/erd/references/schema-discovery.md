...
- **노트(`.note`) 배치** — 겹침 해소는 노트를 **고정 장애물**로 포함한다(노트는 안 움직이고
  겹친 카드가 노트 아래로 밀린다) + 노트가 z-index 로 카드 위에 그려져 주석 텍스트가 카드
  뒤로 숨지 않는다. 그래도 노트는 **카드가 없는 빈 여백(gutter)에 둔다** — 카드 밀집 구역
  한복판에 두면 여러 카드가 크게 밀려 레이아웃이 흔들린다. 노트 폭 250px·높이(줄 수)를
  가늠해 인접 카드와 겹치지 않는 여백을 고른다.
