...
인터뷰는 하나의 도구로 돌아간다: 여섯 가지 고전적 Socratic 질문 유형. 이 파일이 그
여섯 유형의 SSOT 다(`SKILL.md` 는 포인터만) — 유형별 예시 모음, 가장 약한 차원에 맞는
...
ambiguity 가 정체되면(**stalled** — `scoring.md`: 2라운드 연속 <2pt), 막힌 것은 대개
누락된 사실이 아니라 잘못된 가정이다. stalled 가 될 때마다 아래 순서로 다음 모드를
한 번씩 발동하라 — 라운드 번호가 아니라 관측된 정체가 트리거다. 여섯 유형을 대체하지
않는다 — 그것은 여전히 한 개의 유형화된 질문을 던지면서 취하는 *자세*다.

- **1st stalled — Contrarian.** 하중을 지는 가정을 골라 직접 공격한다(유형 2/4 사용).
...
- **2nd stalled — Simplifier.** 쌓여가는 복잡성이 제값을 하는지 탐색한다(유형 4/5).
  오프너: *"우리는 [components/cases] 를 더했습니다. [가장 무거운 것] 을 잘라내면,
  구체적으로 무엇이 실패하나요? 더 멍청한 버전을 먼저 출시할 수 있나요?"*
- **3rd stalled — Ontologist.** 이만큼 와서도 흐릿하다면, 프레이밍이 아마 틀린
  것이다. 핵심 *엔티티*와 그 관계를 중심으로 재구성한다(유형 1/6). repo 에
  `CONTEXT.md` 가 있으면 이 모드 대신 `mattpocock-skills:domain-modeling` 을 호출한다 —
  용어 충돌·과부하 명사·코드 대조가 그 스킬의 본업이고 glossary 갱신까지 이어진다. 오프너:
...
- **결정형** — 답 공간이 닫혀 있다: A 냐 B 냐, 유지냐 변경이냐. grounded brownfield
  ...
  받는 것은 순수 마찰이다. **서로 독립인 결정형은 프론티어로 묶는다**(1콜 ≤4문항 —
  답이 다른 문항의 옵션을 바꾸는 것은 다음 라운드). **첫 옵션 = `(권장)` + 근거 한 줄**
  — 권장의 출처는 코드 현행 동작(`path:line`) 또는 사용자 발화뿐이다. 권장을 못 쓰는
  문항은 결정형이 아니라 조사 부족이다(`[CODE]` 로 되돌려 먼저 캔다).
- **생성형** — 답을 사용자만 만들 수 있다: 구체 입력 예시, 근거·수치, 실제 워크플로
  묘사(유형 1 의 예시 요구, 유형 3 의 근거, 유형 6). 산문으로 **라운드당 하나**,
  **권장답 없이** 물으라 — 옵션도 권장도 사고를 좁힌다. 여기가 grilling 과 갈리는
  지점이고 이 스킬이 Socratic 인 이유다.
...
생성형 질문의 가장 중요한 메커닉. 묶기("그리고, auth 는, 그리고 scale 은, 그리고 schema
