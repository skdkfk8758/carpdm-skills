...
이전에 끝난 WU 는 한 줄로 묶는다: `이전 완료 N개 — 재판정 안 함`. 끝난 트랙(work-order)도 `<트랙>: 완료 — <근거>` 한 줄.
열린 PR 짝 충돌은 `충돌 0건` 또는 `#a × #b: <경로>` 로 적는다. Linear 연결 후보는 `MM-2 ↔ BE-C1` 형태로 적고 전이는 linear 스킬로 보낸다.
