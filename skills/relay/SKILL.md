...
3. WU 마다 프롬프트 파일을 열어 추출한다: **브랜치**(`git worktree add -b <branch>`), **선행**, **소스 경로**(백틱 안 파일 경로), **성공 기준**, **검증 명령**(성공 기준 안의 `make …`·`pytest …`·`npm …`·`curl …`), **담당**(editor·fixer·implementer·scout·[HUMAN]), **멈춤 조건**(파일 수 상한 등 — Step 3 범위 판정에 쓴다).
   - 자기 `worktree add` 줄이 없는 프롬프트는 "같은 워크트리·브랜치" 문구나 work-order 의 산출 PR 열로 **형제 WU 의 브랜치를 상속**한다(여러 WU 가 PR 하나로 나가는 경우). 상속도 안 되면 `확인 필요`.
4. 미완 WU 가 하나라도 있는 work-order 만 **활성**이다. 다음 중 하나면 끝난 트랙으로 보고 표에서 한 줄로 접는다: work-order 에 전부 완료 표기가 있다 · 모든 WU(상속 포함)가 `머지됨` · 트랙이 가리키는 SPEC 의 상태가 `Implemented`. 표 형식이 달라 WU 를 못 뽑은 work-order 도 이 규칙으로 먼저 거른다. 활성이 없으면 최근 머지·열린 PR 만 요약하고, 다음 계획은 goal-prompt·deep-plan 으로 라우팅한 뒤 끝낸다.
...
- Linear: `~/.claude/linear-repo-map.json` 의 `teamRoutes[].repo` 가 이 레포인 팀이 있고 Linear MCP 가 있으면 그 팀의 미완 이슈를 읽는다. 브랜치명에 이슈 ID 가 있으면 직접 잇고, 없으면 이슈 제목과 WU 의 소스 경로·목표를 대조해 **연결 후보**로만 적는다(예: "MM-2 ↔ BE-C1"). 상태 전이는 하지 않는다. 매핑이나 MCP 가 없으면 `Linear: 건너뜀(사유)` 한 줄.

완료 기준: 활성 WU 전부에 상태가 붙어 있고, `머지됨` 은 머지 SHA 가, `PR 열림` 은 번호가 있다.

### Step 3 — 판정

`머지됨` WU 와 `PR 열림` WU 를 판정한다. 돌아온 주자는 대개 PR 을 열어 두고 머지 승인을 기다리므로, **열린 PR 판정이 relay 의 본업**이다. 열린 PR 은 PR head 기준(`git fetch origin '+refs/pull/*/head:refs/remotes/origin/pr/*'` 후 `origin/pr/<번호>`)으로 본다.

- **증거 대조**: PR 본문·커밋이 주장한 검증(명령·수치)을 성공 기준 항목과 짝짓는다. 기준에 있는데 본문에 없는 증거는 `증거 없음` 으로 적고 Step 4 에서 재실행하거나 `실패` 로 둔다.
- **범위**: PR 변경 파일을 프롬프트의 소스 경로·멈춤 조건과 대조한다. 상한을 넘거나 범위 밖 파일이 있으면 관찰에 적는다.
- **충돌**: 열린 PR 끼리 `git merge-tree --write-tree --name-only origin/pr/<a> origin/pr/<b>` 로 짝마다 시뮬레이션한다. 같은 새 파일을 여러 PR 이 추가하면 내용이 같은지 `diff` 로 확인한다.

성공 기준은 항목별로 이렇게 판정한다.
...
완료 기준: `머지됨`·`PR 열림` WU 전부가 판정표에 있고, 모든 행에 명령과 관측이 있으며, 열린 PR 짝마다 충돌 여부가 적혀 있다.
...
1. **준비 집합**: 선행이 모두 `머지됨` 인 미착수 WU. 트랙 간 선행("다른 트랙과의 연결")도 선행으로 친다. 판정을 통과한 `PR 열림` WU 는 **머지되면 풀리는 WU** 로 따로 적고("#119 머지 → FE2 준비"), 머지는 land 로 보낸다. 판정에 실패한 PR 은 같은 주자에게 되돌릴 수정 목록을 적는다.
