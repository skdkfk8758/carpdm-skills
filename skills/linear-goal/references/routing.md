# Routing — repo 확정 + goal/oversized/spec-thin 판정 (dispatch SSOT)

Phase 2 에서 읽는다. **이 파일이 linear-goal dispatch 라우팅의 SSOT 다** — 어느
repo 에서 작업할지(Step 0)와 이 티켓이 goal 로 굴려도 되는 crisp 한 것인지 goal 로는
너무 큰(oversized) 것인지, 아니면 spec 보강이 선행돼야 하는지(Step 1)의 로직 본체가 여기
...
## Step 1 — goal/oversized/spec-thin 판정

위에서 아래로 평가, 먼저 확정되는 곳에서 멈춤:

| 순위 | 신호 | 판정 |
|---|---|---|
| 1 (override) | 라벨 `agent:harness` (레거시 라벨명 — 워크스페이스에 이미 붙어 있어 그대로 인식) | → **oversized** |
| 1 (override) | 라벨 `agent:goal` | → **goal** |
| 2 (→oversized) | estimate set 且 ≥ 5 | → oversized |
| 2 (→oversized) | `area:*` 라벨 2개 이상 (cross-cutting) | → oversized |
| 2 (→oversized) | 제목/본문 regex `(?i)renew|redesign|rework|overhaul|마이그|migration|전면|research|investigate|spike|epic` | → oversized |
| 2 (→oversized) | 비가역 신호 — 본문이 테이블 DROP·대량 데이터 삭제·prod 스키마 apply·다중 env 동시 적용을 명시 | → oversized |
...
| 4 (uncertain) | 위 어디도 확정 안 됨 | → **oversized** (안전 기본값) |
...
- **‡ 대상 특정 가능** = 구체 파일/엔드포인트/함수가 본문에 명시됨, **또는** 범위가
  ...
  oversized 로 보내지 않기 위함. **단 가드 필수**: estimate≥3·범위 모호·AC 비이분법 중
  하나라도면 후자 불성립 → 순위4 uncertain→oversized. 즉 완화는 **좁은 범위 + 이분법
  AC 동시 충족** 일 때만이고, 안전 비대칭(의심 시 oversized)은 그대로 유지된다.

### 핵심 비대칭 — uncertain → oversized (goal 아님)

쉬운 이슈를 oversized 로 보내면 분해 오버헤드(쌈). **어려운 이슈를 goal 보내면
...
반대 방향은 위 rubric 그대로다: **oversized/spec-thin 판정 이슈를 goal 로 강행하지
...
- **oversized** → **여기서 멈춘다**. 본 스킬은 goal 경로 전용이다. 출력:
  `ADT-300 → repo: <repo> / 판정: oversized-class (사유: …) — goal 부적합. /deep-plan 으로 플랜 산출 후 /linear-register 분할 모드로 쪼개 재실행 권장.`
  쪼갠 sub-issue 각각은 다시 이 rubric 을 타 goal 판정을 받는다 — 그게 정경로다.
...
- **spec-thin** → **여기서 멈춘다**. 실행이 아니라 **보강이 선행**돼야 한다. 출력:
  ...
  보강 후 재판정이 정경로 — goal 강행은 oversized 와 동일하게 명시 override 시에만.
