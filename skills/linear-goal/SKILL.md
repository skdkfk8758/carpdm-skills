...
- **UI 작업이면 인수 검증을 Verification 에 박는다 (hard gate).** 이슈가 사용자 대면 UI
  ...
  비 UI 이슈면 생략.
- **UI 작업이면 체크리스트 인계도 `## Done & Report` 에 박는다 (같은 hard gate).** 위 항목이
  *동작 검증*을 지시한다면 이 항목은 *남은 것의 인계*를 지시한다 — 둘은 별개이고, 앞만 넣으면
  worker 가 Part A~C 를 완주하고도 §5 를 흘린다(ADT-406 실측: 정확히 이 형태로 누락). 그대로
  넣을 문장:

  > 사람이 직접 확인할 항목(`[HUMAN]`·`blocked`)이 **2건 이상** 남으면 종료 보고에 텍스트로만
  > 남기지 말고 `ui-verify.md` §5·§5.1 대로 **인터랙티브 체크리스트를 Artifact 로 publish** 하고,
  > 그 전에 작업 워크트리의 dev 서버를 `dev-server-daemon` 으로 **띄운 채** 넘겨 링크가 바로
  > 열리게 하라. 보고 마지막에 artifact URL + dev origin + 정지 명령을 적어라. 체크는 사람이
  > 채운다 — 대신 켜지 마라.
