...
- **UI 작업이면 인수 검증을 Verification 에 박는다 (hard gate).** 이슈가 사용자 대면 UI
  ...
  비 UI 이슈면 생략.
- **체크리스트 인계는 `## Done & Report` 에 항상 박는다 (같은 hard gate — 표면 무관).** 위
  항목이 *동작 검증*(UI 한정)을 지시한다면 이 항목은 *남은 것의 인계*를 지시한다 — 둘은
  별개이고, 앞만 넣으면 worker 가 Part A~C 를 완주하고도 §5 를 흘린다(ADT-406 실측: 정확히 이
  형태로 누락). **비UI 이슈에서도 뺄 수 없다** — worker 는 백그라운드라 `[HUMAN]` walk 이
  불가해 확인 항목이 오히려 더 남고, API·CLI 도 사람이 때려볼 서버가 필요하다. 그대로 넣을 문장:

  > 사람이 직접 확인할 항목(`[HUMAN]`·`blocked`)이 **2건 이상** 남으면 종료 보고에 텍스트로만
  > 남기지 말고 `ui-verify.md` §5·§5.1 대로 **인터랙티브 체크리스트를 Artifact 로 publish** 하라
  > (UI 든 API·CLI 든 표면 무관 — 비UI 면 항목이 실행할 명령과 기대 출력이 된다). **1건이어도**
  > 작업 워크트리의 dev 서버를 `dev-server-daemon` 으로 **띄운 채** 넘겨 링크가 바로 열리게 하고,
  > 보고 마지막에 artifact URL(냈으면) + 실측 dev origin + 정지 명령을 적어라. 띄울 dev 명령이
  > 없는 프로젝트면 기동을 생략하고 서버 없이 실행 가능한 확인 방법을 적어라(§5.1-0). 체크는
  > 사람이 채운다 — 대신 켜지 마라.

  worker 가 워크트리에서 돌므로 `--cwd` 는 **그 워크트리**여야 한다(메인 체크아웃을 띄우면
  사람이 *변경 없는 브랜치* 화면을 검증한다).
