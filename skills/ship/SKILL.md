---
name: ship
description: carpdm-skills 레포 전용 — 라이브 `~/.claude/skills/` 의 스킬 변경을 repo 로 미러하고, PR 을 올리고, CI 통과를 기다린 뒤, 한 번의 승인 게이트를 거쳐 머지하고 로컬을 재동기화한다. 즉 "PR 올리고 land 까지" 를 한 흐름으로 잇되 머지는 CI 게이트 + 명시 승인 뒤에만 한다. 사용자가 이 레포에서 스킬을 편집한 뒤 그것을 배포·반영하고 싶어 할 때 사용 — "이 스킬 변경 PR 올리고 머지까지", "ship 해줘", "PR 올리고 land 까지 가줘", "스킬 동기화 배포", "CI 통과하면 머지하고 정리" 같은 표현. `sync.sh --push`(PR 생성과 동시에 즉시 머지 — CI·승인 게이트 없는 빠른 경로)와 다르다: ship 은 CI 를 기다리고 머지 전 한 번 멈춰 확인한다. 스킬 내용을 작성·수정하거나(직접 SKILL.md 편집), 이미 머지된 여러 PR/워크트리를 범용으로 정리(use land)하는 데에는 쓰지 말 것 — ship 은 *이 레포의 스킬 변경 한 건* 을 PR→CI→머지→로컬정리로 흘려보내는 배포 스킬이다.
---
