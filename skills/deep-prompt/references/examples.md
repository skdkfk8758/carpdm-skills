# deep-prompt — 작동 예시 & 검증 가능성 루브릭

SKILL.md 가 Success Criteria 를 못 박을 때 참조. 좋은 goal 프롬프트 2개, 그리고
약한 것을 고친 before/after 1개.

## 검증 가능성 루브릭

Success Criteria 한 항목을 저장 전에 이 다섯 질문에 통과시킨다:

1. **관찰 가능한가** — 사람 판단 없이 명령 출력·파일 내용·종료 코드로 참/거짓이
   갈리는가? ("안전해짐" ✗ / "11번째 요청에 429" ✓)
2. **Verification 짝이 있나** — 이 기준을 확인할 정확한 명령/관찰이 Verification
   섹션에 있나? 짝 없으면 에이전트가 체크 못 한다.
3. **에이전트 권한 안인가** — 에이전트가 가진 도구·접근으로 실제로 확인 가능한가?
   (외부 대시보드를 봐야 알 수 있으면 자율 판정 불가.)
4. **이분법인가** — 부분 충족이 모호하지 않은가? "대부분 테스트 통과"는 회색
   지대 — "모든 테스트 통과"로.
5. **루프를 끝내나** — 이 기준이 다 참이면 에이전트가 *멈춰도 되는가*? 멈춤을
   정당화 못 하는 기준은 빠진 게 있는 것.
...
## Done & Report
완료 시: 근본 원인 한 줄 + 변경 파일 목록 + 회귀 테스트 경로 보고.
막히면(재현 불가 등): 무엇을 시도했고 무엇이 필요한지 명시.
```

왜 자율 실행되나: 세 기준 모두 명령으로 판정된다. 가드레일이 push·마이그레이션을
막는다. 재현 불가 시 빠져나올 신호가 있다.
...
## Done & Report
완료 시: 추가한 행과 갱신한 카운트 보고.
가드가 여전히 실패하면: 어느 스킬이 누락 판정됐는지 출력.
```
