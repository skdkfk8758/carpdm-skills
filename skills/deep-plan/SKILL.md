...
가중치여야 자기채점이 진짜 검증이 된다. 단 codex 는 여기서 **메타프롬프팅
비평자**로만 쓴다 — craft 빌드 파이프라인의 *적대적 플랜 리뷰*(Phase 2, 수렴
핑퐁)는 돌리지 않는다. TDD 도, 보안 페이즈도, 구현 코드도 없다.
...
- **vs `forge`/`renew`/`hunt`** — 그쪽은 빌드한다. deep-plan 은 구현 코드를 한 줄도
  쓰지 않는다. codex 수렴 핑퐁 플랜리뷰도 그쪽 Phase 2 소관.
...
규칙)뿐이다. 그 파일의 적대적 리뷰 프롬프트·수렴 핑퐁 계약은 이 스킬 것이 아니다 —
가져오지 말 것.
...
> 시 `'opus'` 재시도 — 폴백 사실 보고)로 띄운다. Step 5+6 의 PLAN·시안 작성은
> 종전대로: 세션이 최상위 티어(fable/opus 급)면 메인 인라인, 아니면 fable
> 서브에이전트 위임.
...
2. **`[HUMAN]` 갭만 사용자에게 — 라운드당 한 질문.** 절대 묶지 말 것.
   - **결정형**(답 공간이 닫힘) → `AskUserQuestion`. **Anchoring 가드**: 옵션은
     사용자가 이미 말한 것 또는 코드 실측에서만 도출. 출처 없으면 생성형이니 산문.
   - **생성형**(구체 예시·수치·근거·워크플로 묘사) → 산문.
3. **이미 답한 것을 다시 묻지 않는다.** 한 답이 다른 갭까지 닫으면 그 사실을
   말하고 그 갭도 닫는다.
...
- **④ codex 최종 verdict** — `task --resume-last` 로 같은 스레드 재개, 최종본을
  넘겨 **자기 BLOCKING·갭이 실제로 닫혔는지 항목별 verdict** 를 받는다. 저자와
  채점자가 다른 모델이라 이 채점이 진짜 교차검증이다.
