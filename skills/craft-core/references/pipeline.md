...
- **UI 인수 검증 게이트 — 인터랙션 실구동 + 시안 갭 분석 (hard gate).** 이번 diff 가
  ...
  - **§5·§5.1 체크리스트 인계** — 남은 `[사용자 직접 확인 필요]` 가 2건 이상이면 인터랙티브
    체크리스트를 Artifact 로 publish 하고, **1건이어도 dev 서버는 띄운 채** 넘겨 링크가 바로
    열리게 한다(실측 origin — 관례 포트 추정 금지). Part A~C 를 완주하고 이것만 흘리는 게
    실측된 누락 형태다(ADT-406) — 앞 셋이 *검증*이라면 이건 *인계*이고, §4 게이트의 통과
    요건이다.
...
- **인계 (§V 에 사람 몫이 남을 때 — 표면 무관).** `[사용자 직접 확인 필요]` 가 남으면
  블록만 내고 끝내지 않는다: **2건+ 면** 인터랙티브 체크리스트를 Artifact 로 publish 하고,
  **1건이어도** 작업 워크트리의 dev 서버를 daemon 으로 띄운 채 실측 origin 을 그 항목의
  확인 방법에 박는다(`ui-verify.md` §5·§5.1 — 절차 SSOT, 복제 금지). UI 빌드는 Phase 4 에서
  이미 했을 테니 **중복 publish 금지**(그 URL 을 그대로 잇는다) — 비UI 빌드(API·CLI·배치)는
  Phase 4 가 ui-verify 를 안 읽으므로 **여기가 유일한 진입점**이다. 띄울 dev 명령이 없는
  프로젝트면 기동을 생략하고 서버 없이 실행 가능한 확인 방법을 적는다(§5.1-0).
