...
**In Review 전이는 PR 생성 *뒤*에 (순서 실측, 2026-07-30).** GitHub 연동 자동화는
"PR open → In Progress" 규칙을 갖는 경우가 많다 — In Review 로 먼저 옮기고 나서
PR 을 만들면 그 자동화가 **수동 전이를 1분 안에 덮는다**(실측 AUT-73: 22:30 In
Review 전이 → 22:31 PR 생성 → 22:31:49 In Progress 회귀). wrap 에서 PR 까지 만드는
플로우면 **push+PR 생성 → In Review 전이** 순서로. PR 을 안 만들고 끝나는 wrap 은
영향 없음(이벤트가 없다). 이미 덮였으면 재전이 1회로 복구된다 — 다음 PR-open
이벤트는 없으므로 유지된다.
