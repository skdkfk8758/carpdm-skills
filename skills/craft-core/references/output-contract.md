...
**박스 문자·고정폭 정렬 금지 (2026-07-29).** 종전 `┌─└` 프레임 + 공백 정렬은 렌더
폭에 의존해 내용이 랩핑되면 테두리·정렬이 깨진다(실측 스크린샷). 보드는 **plain
markdown 리스트 + 볼드 라벨**로 emit 한다 — 폭 무관하게 랩핑돼도 행 정체가 산다:
...
- 한 행이 길어지면 하위 불릿으로 내려쓴다(변경 행의 파일별 1행과 같은 꼴) —
  한 줄에 우겨넣고 랩핑에 맡기지 않는다.
- Linear 코멘트(규칙 6 재사용)에서도 같은 markdown 이 그대로 렌더된다 — 박스
  문자였을 때는 코멘트에서도 깨졌다.
...
6. **§N 잔여 행과 중복 금지.** 잔여 행은 이 블록을 포인터로 참조한다
   ...
   금지, §N 규칙 5 동형).
7. **`[사용자 직접 확인 필요]` 2건+ 이면 인터랙티브 체크리스트 아티팩트로도 배출한다**
   (`ui-verify.md` §5 — 템플릿·트리거 SSOT). **표면 무관** — UI 든 API·CLI·배치든 사람 몫
   확인이 2건 이상이면 모은다(비UI 는 `ui-verify.md` §0 의 §5 예외로 진입: Part A~C 는
   건너뛰고 §5·§5.1 만). 이 텍스트 블록은 요약, 아티팩트가 조작면 — 항목을 양쪽에 이중
   나열하지 않고 §V 아래(또는 §N `열기` 행)에 URL 만 건다. agent 가 대신 체크 금지(사람이
   채운다).
7-1. **확인 항목이 1건이라도 남으면 dev 서버를 띄운 채 넘기고 실측 origin 을 적는다**
   (`ui-verify.md` §5.1). 아티팩트를 걸 땐 URL 옆에 `dev origin` + `정지 명령`을 같은 줄에,
   **아티팩트 없이 1건일 때는 그 항목의 "확인 방법" 구절에** origin 을 박는다. 확인 표면이
   죽어 있으면 사람은 첫 항목에서 `ECONNREFUSED` 다. origin 은 daemon 이 실제로 찍은 값만
   (관례 포트 추정 금지 — 메인 체크아웃의 다른 브랜치를 가리키면 사람이 변경 없는 화면을
   검증한다). 띄울 dev 명령이 없는 프로젝트(라이브러리·CLI)면 기동을 생략하고 서버 없이
   실행 가능한 확인 방법을 적는다(§5.1-0).
