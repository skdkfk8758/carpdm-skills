...
**시간 가드레일 (실측)**: implement 에이전트 1회 벽시계 ~10분(600s) 초과 =
그 태스크는 atomic 이 아니었다는 사후 신호다 (실측: 647s/561s/530s 태스크들이
한 워크플로 벽시계의 75% 를 차지). 태스크를 자를 때 "이게 opus 런 10분 안에
red→green→refactor 가 되나?" 를 기준으로 삼아라. 넘칠 것 같으면 설계 단계에서
더 잘게 — 사후에 발견하면 다음 유사 태스크 분할에 반영.
