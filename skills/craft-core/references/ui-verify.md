...
없고 AND §5 트리거가 걸렸으면 체크리스트 아티팩트가 실제로 publish 됐을 것.**
...
— 사람이 dev 서버를 보며 눌러 채우는 **인터랙티브 체크리스트를 Artifact 로 publish** 한다.
백그라운드 잡(walk 불가로 전 항목이 `[사용자 직접 확인 필요]`)·다이슈 묶음(linear-goal /
land 꼬리)에서 특히 그렇다 — 흩어진 수동 확인 항목을 사람이 놓치지 않게 한 화면에 모은다.

- **트리거**: UI 표면 변경 + `[사용자 직접 확인 필요]`(=미검증 `[HUMAN]`·`blocked`) 2건+.
  1건이면 §V 텍스트 한 줄로 충분 — 아티팩트는 과함(karpathy 원칙 2).
- **재료**: `~/.claude/skills/craft-core/references/assets/human-checklist-template.html` 를
  ...
  저장·진행률이 이미 들어있다, 새로 디자인하지 말 것). 항목 = ui-verify Part A 인벤토리 중
  `blocked` + 미검증 `[HUMAN]` 만(이미 `pass` 한 `[직접 테스트 완료]` 는 제외 — 사람이 다시
  누를 필요 없음). 각 항목은 관찰 가능한 성공 신호로(§4 증거 규율 그대로).
- **서버 선행 기동 (필수)** — 아래 §5.1. 체크리스트는 *사람이 눌러 볼 화면*을 전제하므로,
  띄워두지 않으면 첫 항목에서 막힌다.
...
### 5.1 dev 서버를 띄운 채 넘긴다 (체크리스트를 낼 때 필수)

체크리스트만 주고 서버를 내리면 사람은 링크를 눌러 `ECONNREFUSED` 를 만난다. 아티팩트를
낼 때는 **작업한 브랜치/워크트리의 dev 서버를 살려둔 채** 넘긴다.
...
3. **그 origin 을 아티팩트에 박는다** — `{{DEV_ORIGIN}}`·route 링크·foot. 서버가 끝내
   안 뜨면 **링크를 만들지 않고** 라우트 경로만 텍스트로 두고, `{{SERVER}}` 블록에 기동
   명령과 실패 사유를 적는다(죽은 링크가 붙은 체크리스트가 제일 나쁘다).
