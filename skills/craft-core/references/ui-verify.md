...
1. **daemon 으로 띄운다** — `run_in_background` 나 `&`/`nohup` 은 턴 종료 시 tree-kill 로
   ...
   ```bash
   python3 ~/.claude/skills/dev-server-daemon/scripts/devserverctl.py start --cwd <작업 워크트리 절대경로>
   ```

   **`--cwd` 는 반드시 이번에 바꾼 코드가 있는 워크트리**다 — 메인 체크아웃(다른 브랜치)을
   가리키면 사람은 *변경이 없는 화면*을 보며 체크하게 된다. 포트가 이미 물려 있으면
   `--cmd "PORT=<빈포트> make dev"` 로 비껴간다.
2. **origin 을 실측한다** — 스크립트가 찍은 URL·포트를 그대로 쓴다. 관례값(`:3000`)을
   가정하지 않는다. 안 떴으면 `logs` 로 확인하고, 그래도 안 되면 3항으로.
