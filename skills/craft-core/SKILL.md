---
name: craft-core
description: forge / renew / reshape / hunt 개발 스킬을 위한 내부 공유 엔진. 직접 호출하지 않음 — 해당 스킬들이 이 reference 파일들을 읽는다.
...
(codex) → opus 에서의 dynamic-workflow TDD → 보안 검증**. 4종의
작업유형 스킬(`forge`, `renew`, `reshape`, `hunt`)은 각자 자신의
