---
name: loop-harness-setup
description: 프로젝트·레포에 개발 "하니스"(loop / eval-게이트 자율개발 오케스트레이터 인프라)를 **처음 설치·셋업·이식·복제**하려 할 때 쓴다. 신호는 단 하나 — 타깃 레포에 하니스가 *아직 없어* 새로 깔아야 함. 여기엔 다른 레포(가령 Intelligence-Auth)의 하니스를 그대로 가져오기, 거기에 loop/ 가시화 HTML·일별 로그까지 함께 세팅하기가 포함된다. 예 — "이 레포/프로젝트에 (루프) 하니스 깔아줘·셋업·설치", "하니스 이식·복제해줘", "그 레포 하니스 그대로 가져와줘", "harness-run 돌리기 전에 하니스부터 깔아줘", "가시화·로그까지 한번에", "eval 게이트 자율개발 하니스 세팅", "set up / port / copy the loop harness in this repo". 이미 깔린 하니스로 이슈를 *실행*(harness-run)·버그수정(hunt)·기능빌드(forge)·플랜(deep-plan)하는 것과 다르다 — 이 스킬은 하니스 인프라 자체를 새로 설치한다.
---
