...
- 완료 조건은 그대로 Success Criteria가 되므로 사람 없이 판정 가능한 것만 쓴다. 제품 조작·명령과 그
  관측 결과(엔드포인트 응답, 화면 상태, CLI 출력), 파일 존재, 수치. 테스트·타입체크·린트 명령은 검증
  수단이지 성공의 정의가 아니므로 쓰지 않는다. 주관 판정과 외부 승인 항목은 `## 범위 밖` 또는
  `## 참고`로 옮긴다.
