...
## 4. 게이트 병합 표시 — 2단 (본문 미리보기 → 선택지)

Step 3 확인 게이트는 두 단으로 나뉜다. **1단은 `AskUserQuestion` 직전 메시지**로,
생성될 **본문 전문**을 이슈별 fenced 블록에 담아 dedup·배치 정보와 함께 보여준다:
...
**왜 본문 전문인가**: 제목만 보여주는 게이트는 분량·문체 위반을 막지 못한다 — 실측으로
2,300자 이슈가 제목만 표시된 게이트를 통과했고, 위반은 등록된 뒤에야 보였다. 사람이
승인 전에 읽을 수 있어야 본문 계약(SKILL.md §분량 상한·§문체)이 실효를 가진다.

**2단은 `AskUserQuestion` — 선택지만.** 본문을 선택지 텍스트에 넣지 않는다(들어가지도
않고, 1단이 이미 보여줬다). 유사 후보가 있는 이슈는 이슈별 4택:
