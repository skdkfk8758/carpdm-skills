...
name: cicd-scaffold
description: >-
  Use this skill to GENERATE GitHub Actions deploy workflows (.github/workflows/)
  for a Node app — when the user wants to set up, build, scaffold, or wire up
  CI/CD or a deploy pipeline, not debug one. Target shape: push to develop
  auto-deploys to dev, a version tag deploys to prod (the manual tag is the gate;
  optional Required-reviewers approval on Pro/Team), images build and push to AWS
  ECR (often via OIDC, no static keys), running on self-hosted runners (zero
  GitHub-hosted minutes). Single repo or monorepo (api + web); Next.js, Express,
  similar Node. Trigger whenever the user asks to create deploy automation
  matching that shape — even partially, even without saying "CI/CD":
  "build the github actions to deploy to dev and prod", "ship to prod on a tag
  and dev on develop", "continuous deployment with ECR and a self-hosted runner".
  It detects build/test commands and ports, fills the AWS/GitHub gaps, then
  writes the workflows, a Dockerfile if missing, and a setup checklist. Do NOT
  use to debug an existing pipeline, write a standalone Dockerfile or deploy
  script, or provision Terraform/IAM alone.
---
