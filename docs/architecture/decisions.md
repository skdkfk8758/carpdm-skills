# Architecture — 설계 결정 (§1~§16)

> `CLAUDE.md` 에서 disclose 된 reference. **절 번호는 상호참조가 걸려 있어 고정** —
> 절을 지울 때도 번호는 남기고 본문만 은퇴 기록으로 대체한다(§13 이 그 선례).
> 이 파일은 항상 로드되지 않는다 — CLAUDE.md 의 라우팅 표가 가리킬 때만 읽는다.
