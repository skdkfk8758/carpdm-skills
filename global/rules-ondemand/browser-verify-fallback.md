...
IMPORTANT: 브라우저 자동화는 검증 **수단**이지 검증 **자체**가 아니다. 실측: Chrome extension bridge 가 5회 재시도 내내 실패해 수동 DevTools 측정으로 넘어가기까지 시간을 통째로 태웠다. **도구가 막히면 빨리 포기하고 대체 경로로 검증을 완수한다** — 재시도는 검증이 아니다.

> 1차 도구는 **aside MCP**(`mcp__aside__repl`, Playwright 내장)다. claude-in-chrome 통합은
> 비활성(`claudeInChromeDefaultEnabled: false`) + 훅(`guard-chrome-to-aside.sh`) 이중 차단 —
> 실제 Chrome 로그인 세션이 명시 요구될 때만 설정 `true` + `touch ~/.claude/.allow-chrome-mcp` + 새 세션, 끝나면 원복.

## 규칙

1. **2회 캡.** 같은 브라우저 도구 호출이 2회 연속 실패(무응답·bridge 에러·탭 invalid)하면 그 도구 재시도를 중단한다. 3회째 동일 호출 금지.
2. **Fallback 사다리 (위에서 아래로).**
   - aside MCP(`mcp__aside__repl`) 실패 → **Playwright MCP / playwright-cli** (독립 브라우저).
...
- aside MCP (`mcp__aside__repl`) — 1차 브라우저 도구 (글로벌 CLAUDE.md §브라우저).
