...
> **Scope default — current linked team first.** Prefer the current worktree's linked team/workspace (from `orca linear issue --current --json`) over a blanket `--workspace all`. Reach for `--workspace all` only when the ticket genuinely lives outside the current link and you cannot resolve it otherwise — a whole-workspace sweep is noisy (aligns with the `linear-dispatch.md` current-repo-team scoping principle).
