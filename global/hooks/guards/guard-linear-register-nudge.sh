...
# through the `linear-register` skill. NON-BLOCKING — exit 0 + additionalContext
# (lib-emit-context.sh).
#
...
{
echo "[guard] NUDGE: Linear 이슈 생성 감지 — linear-register 스킬 경유했나?"
echo "필수 절차: ① repo→팀 라우팅(linear-repo-map) ② 생성 전 확인 게이트"
echo "③ 이슈마다 '## 추천' 섹션 ④ 체인이면 관계+전방 kickoff 프롬프트. (이미 경유 중이면 무시)"
