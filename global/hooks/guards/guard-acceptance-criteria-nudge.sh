...
# exit 0 + additionalContext (lib-emit-context.sh).
#
...
#   GUARD_LINEAR_ISSUE_RE=<regex>      — issue-id pattern (shared default: team keys, see lib-issue-id.sh)
...
# Ids match only real tracker team keys (shared with guard-linear-state-nudge).
. "$(dirname "${BASH_SOURCE[0]}")/lib-issue-id.sh"

# Primary signal: current branch name (deliberate, e.g. feat/adt-196-...).
# Fallback: the command text.
...
if [ "$BOUNDARY" = "pr_create" ]; then
  echo "[guard] NUDGE: $ISSUE_UC PR 생성 — 수용 기준 검증했나? (CLAUDE.md §검증)"
  echo "각 항목을 관찰 가능하게 검증 후에만 체크: [AUTO]는 테스트, [HUMAN]은 런타임/시각 확인."
  echo "검증 없이 [x] 금지(거짓 완료). 미검증 [HUMAN] 있으면 PR/Done 보류하고 사용자 안내."
else
  echo "[guard] NUDGE: $ISSUE_UC 머지 — 수용 기준 100% 검증·체크됐나? (CLAUDE.md §검증)"
  echo "모든 [AUTO] green AND 모든 [HUMAN] 사용자와 walk(pass/명시 수용)여야 Done."
  echo "미검증·미체크 항목이 남았으면 Done 보류 — 거짓 완료가 회귀로 새어나간다(SUR-26)."
fi
