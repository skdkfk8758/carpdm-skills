...
description: Imported Claude skill — single entry point for registering Linear issues (single/few native registration and plan/spec/PRD split registration), with dedup against existing backlog, grouping proposals, and per-issue recommendation sections.
---

> **Codex 런타임 노트** (본문은 Claude 원본 verbatim — 아래만 치환해 읽는다):
> - `mcp__linear__*` 도구명 → Codex 에서도 동일 이름으로 노출됨(실측 2026-08-01) — 그대로 호출. 이름이 안 보이면 available tools 확인 후 대응(추측 호출 금지).
