...
- **다음** — 이 작업 이후 후보 최대 3개 `<할 일> — <왜 지금> → /<스킬>` · 후보 0건이면 행 생략
```
...
**다음 행은 잔여와 겹치지 않는다** — `[필수]` 잔여는 이번 작업 몫이라 다음에서 제외한다. 후보는
세션 컨텍스트 먼저, 0건일 때만 Linear(연결돼 있을 때만, 없으면 행 생략) — 상세는 response-format.md §다음 작업.
